...
- 入库建单、整单收货、库存余额与不可变库存流水联动。
- 库存查询、调整、移库和库存流水分页检索。
- 出库建单、FEFO 库存分配、发运扣减；过期库存和停用货位不参与分配。
...
仍未实现的重点能力包括部分收货、质检与上架任务，出库取消和预占释放，拣货/复核/包装，盘点、冻结/解冻、退货和红冲，多货主与序列号，PDA/条码/标签，波次与补货，细粒度权限和仓库数据范围，以及共享令牌撤销、OpenAPI、审计中心、指标告警和消息事件。当前版本适合开发、课程设计和受控演示；完成上述生产治理项前不应直接作为无人值守的大规模生产 WMS。
