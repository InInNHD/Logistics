...
class JwtGatewayFilter implements GlobalFilter, Ordered {
    ...
    private final TokenStatusClient tokenStatus;
    JwtGatewayFilter(JwtService jwtService) { this(jwtService, null); }
    @Autowired JwtGatewayFilter(JwtService jwtService, TokenStatusClient tokenStatus) {
    ...
    @Override public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            ServerWebExchange current = exchange;
            return (tokenStatus == null ? Mono.just(true) : tokenStatus.active(claims)).flatMap(active -> {
                if (!active) return unauthorized(current, "登录凭证已失效，请重新登录");
                ServerWebExchange authenticated = current.mutate().request(request -> request.headers(headers -> {
    ...
    private boolean isAuthorized(HttpMethod method, String path, String role) {
        ...
        if (HttpMethod.POST.equals(method) && path.equals("/api/auth/logout")) return true;
        ...
        if ("PICKER".equals(role) && HttpMethod.POST.equals(method)) {
            return path.equals("/api/outbound-orders")
                    || path.matches("/api/outbound-orders/\\d+/(allocate|pick|pack|ship|cancel|return)");
        }
        return false;
    }
