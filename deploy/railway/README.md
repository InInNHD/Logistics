...
5. 打开前端域名并登录，完成创建入库单、收货、分配出库和发运的冒烟测试。

部署后立即更换默认管理员密码，并在 Railway 中启用用量上限和数据库备份策略。生产示例将 JWT 有效期设为 1 小时；在共享撤销机制完成前，停用用户或调整角色后，仓储权限最迟会在旧令牌过期时完全收敛。Redis 当前不是运行必需服务，不要在五服务试用项目中额外创建。
