...
当前已补齐账号申请审批、令牌撤销/即时状态校验、认证审计、OpenAPI、分批收货、出库取消释放、拣货包装、整单退货和单维度盘点。仍在路线图中的能力包括质检/上架、部分发运/退货、范围盘点冻结、PDA/条码/标签、波次与补货、多货主/序列号、细粒度权限与仓库数据范围、集中告警、消息事件和多实例服务发现。
