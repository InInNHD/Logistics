...
function readSavedUser(): UserProfile | null {
  const savedUser = localStorage.getItem('firefly_user')
...
export const useAuthStore = defineStore('auth', () => {
  const token = ref(localStorage.getItem('firefly_token') || '')
  ...
  async function signIn(username: string, password: string) {
    ...
    localStorage.setItem('firefly_token', token.value)
    localStorage.setItem('firefly_user', JSON.stringify(user.value))
  }
