...
const dialogType = ref<'adjust'|'transfer'|'stocktake'>('adjust')
const intentKey = ref('')
...
const stocktake = reactive({ inventoryId: undefined as number|undefined, label: '', bookQuantity: 0, actualQuantity: 0, reason: '' })
...
function openStocktake(item:InventoryItem){Object.assign(stocktake,{inventoryId:item.id,label:`${item.sku} · ${item.locationCode} · ${item.batchNo||'无批次'}`,bookQuantity:item.quantity,actualQuantity:item.quantity,reason:''});dialogType.value='stocktake';intentKey.value=crypto.randomUUID();dialogVisible.value=true}
...
async function submitStocktake(){if(submitting.value||!stocktake.inventoryId)return;if(stocktake.actualQuantity===stocktake.bookQuantity)return ElMessage.warning('实盘数量与账面数量一致，无需提交');submitting.value=true;try{await warehouseApi.stocktakeInventory({inventoryId:stocktake.inventoryId,actualQuantity:stocktake.actualQuantity,reason:stocktake.reason},intentKey.value);ElMessage.success('盘点差异已入账');dialogVisible.value=false;intentKey.value='';await load()}finally{submitting.value=false}}
...
function movementTypeLabel(type:string){return {INBOUND_RECEIPT:'入库收货',OUTBOUND_SHIPMENT:'出库发运',OUTBOUND_RETURN:'出库退货',ADJUSTMENT:'库存调整',STOCKTAKE:'库存盘点',TRANSFER_IN:'移库入',TRANSFER_OUT:'移库出'}[type]||type}
  ...
  <div class="page-container">
    ...
    <section class="surface-card table-card">
      ...
      <el-table v-loading="loading" :data="inventory" stripe>
        ...
        <el-table-column label="有效期" width="125"><template #default="s"><span :class="expiryTone(s.row.expiryDate)">{{ s.row.expiryDate||'—' }}</span></template></el-table-column><el-table-column v-if="canManageInventory" label="操作" width="140" fixed="right"><template #default="s"><el-button link type="primary" @click="openTransfer(s.row)">移库</el-button><el-button link type="warning" @click="openStocktake(s.row)">盘点</el-button></template></el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogType==='adjust'?'库存调整':dialogType==='transfer'?'库存移库':'库存盘点'" width="560px" :close-on-click-modal="!submitting" :close-on-press-escape="!submitting" :show-close="!submitting">
      ...
      <el-form v-else-if="dialogType==='transfer'" :model="transfer" label-position="top"><el-form-item label="库存明细"><el-select v-model="transfer.inventoryId" filterable style="width:100%"><el-option v-for="item in inventory" :key="item.id" :label="`${item.sku} · ${item.locationCode} · 可用 ${item.availableQuantity}`" :value="item.id" /></el-select></el-form-item><div class="form-grid"><el-form-item label="源库位"><el-input v-model="transfer.sourceLocationCode" disabled /></el-form-item><el-form-item label="目标库位"><el-select v-model="transfer.targetLocationCode" filterable style="width:100%"><el-option v-for="item in transferLocations" :key="item.id" :label="`${item.code} · ${item.name}`" :value="item.code" /></el-select></el-form-item><el-form-item label="移动数量"><el-input-number v-model="transfer.quantity" :min="1" style="width:100%" /></el-form-item><el-form-item label="移库原因"><el-input v-model="transfer.reason" /></el-form-item></div></el-form>
      <el-form v-else :model="stocktake" label-position="top"><el-alert title="存在已分配或冻结数量的库存不能盘点" type="warning" :closable="false" class="stocktake-alert" /><el-form-item label="库存明细"><el-input v-model="stocktake.label" disabled /></el-form-item><div class="form-grid"><el-form-item label="账面数量"><el-input-number v-model="stocktake.bookQuantity" disabled style="width:100%" /></el-form-item><el-form-item label="实盘数量"><el-input-number v-model="stocktake.actualQuantity" :min="0" style="width:100%" /></el-form-item><el-form-item label="盘点说明" class="full"><el-input v-model="stocktake.reason" placeholder="例如：月度循环盘点" /></el-form-item></div></el-form>
      <template #footer><el-button :disabled="submitting" @click="closeDialog">取消</el-button><el-button type="primary" :loading="submitting" @click="dialogType==='adjust'?submitAdjust():dialogType==='transfer'?submitTransfer():submitStocktake()">确认执行</el-button></template>
    </el-dialog>
    ...
    <el-drawer v-model="movementDrawerVisible" title="库存流水" size="min(1100px, 92vw)" destroy-on-close>
      <div class="movement-toolbar">
        ...
        <el-select v-model="movementType" clearable placeholder="全部类型"><el-option label="入库收货" value="INBOUND_RECEIPT" /><el-option label="出库发运" value="OUTBOUND_SHIPMENT" /><el-option label="出库退货" value="OUTBOUND_RETURN" /><el-option label="库存盘点" value="STOCKTAKE" /><el-option label="库存调整" value="ADJUSTMENT" /><el-option label="移库入" value="TRANSFER_IN" /><el-option label="移库出" value="TRANSFER_OUT" /></el-select>
...
.movement-toolbar{display:grid;grid-template-columns:minmax(240px,1fr) 170px 150px auto;gap:10px;margin-bottom:18px}.positive{color:#347d65}.negative{color:#c55c51}.stocktake-alert{margin-bottom:18px}
